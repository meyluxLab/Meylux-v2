...
class PostClosureSecurityBoundaryTests(unittest.TestCase):
    ...
    def test_governed_grant_hardening_on_real_postgresql(self) -> None:
        ...
        self.assertEqual(
            self._psql(
                "SELECT version FROM meylux.schema_migrations WHERE version IN ('0001_database_foundation','0002_raw_acquisition_staging','0003_canonical_persistence_event_outbox','0004_canonical_quality_state_alignment','0005_quantitative_foundation','0006_application_role_grant_hardening') ORDER BY version;"
            ).stdout.splitlines(),
            [
