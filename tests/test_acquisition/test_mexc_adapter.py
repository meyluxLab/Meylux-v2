...
class MEXCAdapterTests(unittest.TestCase):
    def test_identity_and_capabilities_are_provider_isolated(self):
        ...
        self.assertEqual({c.capability for c in mexc.capabilities()}, {
    ...
    def test_order_book_maps_provider_payload_and_sequence(self):
        def http_get(url, _timeout):
            ...
            return json.dumps({"lastUpdateId": 12345, "bids": [["100", "2"]], "asks": [["101", "1"]]}).encode()
        ...
        self.assertEqual(envelope.provider.provider_id, "mexc")
        self.assertEqual(envelope.instrument.provider_instrument_id, "BTCUSDT")
        self.assertEqual(envelope.source_sequence, "12345")
        self.assertEqual(envelope.payload["bids"][0][0], "100")

    def test_trades_map_time_and_identity_deterministically(self):
        body = [{"id": 7, "price": "100", "qty": "2", "quoteQty": "200", "time": 1757592000000}]
        mexc = adapter(http_get=lambda _url, _timeout: json.dumps(body).encode())
        first = mexc.fetch_trades("BTCUSDT")[0]
        second = mexc.fetch_trades("BTCUSDT")[0]
        self.assertEqual(first.source_sequence, "7")
        self.assertEqual(first.event_id, second.event_id)
        self.assertEqual(first.event_time.tzinfo, timezone.utc)

    def test_klines_preserve_raw_wire_row_without_normalization(self):
        row = [1757592000000, "100", "110", "90", "105", "12", 1757592059999, "1250"]
        mexc = adapter(http_get=lambda _url, _timeout: json.dumps([row]).encode())
        envelope = mexc.fetch_klines("BTCUSDT", "1m")[0]
        self.assertEqual(envelope.event_type, EventType.CANDLE)
        self.assertEqual(envelope.payload["row"][1], "100")
        self.assertEqual(envelope.source_sequence, str(row[0]))

    def test_exchange_info_supports_symbol_and_server_time(self):
        def http_get(url, _timeout):
            self.assertIn("/api/v3/exchangeInfo", url)
            self.assertIn("symbol=BTCUSDT", url)
            return json.dumps({"serverTime": 1757592000000, "symbol": "BTCUSDT", "status": "ENABLED"}).encode()
    ...
    def test_bootstrap_calls_rest_components_without_persistence(self):
        calls = []
        def http_get(url, _timeout):
            calls.append(url)
            if "exchangeInfo" in url:
                return b'{"symbol":"BTCUSDT","status":"ENABLED"}'
            return b'{"lastUpdateId":1,"bids":[],"asks":[]}'
        info, depth = adapter(http_get=http_get).bootstrap("BTCUSDT")
        self.assertEqual(info.state, AcquisitionState.AVAILABLE)
        self.assertEqual(depth.state, AcquisitionState.AVAILABLE)
    ...
    def test_http_429_maps_to_rate_limited_after_bounded_retry(self):
        ...
        self.assertEqual(envelope.provider_error.code, "MEXC_HTTP_429")
    ...
    def test_provider_api_error_maps_to_unavailable(self):
        mexc = adapter(http_get=lambda _url, _timeout: b'{"code":10007,"msg":"bad symbol"}')
        envelope = mexc.fetch_order_book("BAD")
        self.assertEqual(envelope.state, AcquisitionState.UNAVAILABLE)
        self.assertEqual(envelope.provider_error.code, "MEXC_API_10007")

    def test_stream_trade_maps_to_trade_envelope(self):
        message = {
            "channel": "spot@public.deals.v3.api@BTCUSDT",
            "symbol": "BTCUSDT",
            "publicdeals": {"dealsList": [{"price": "100", "quantity": "2", "tradetype": 1, "time": 1757592000000}]},
            "sendtime": 1757592000001,
        }
        envelope = adapter().parse_stream_message(json.dumps(message))
        self.assertIsNotNone(envelope)
        self.assertEqual(envelope.event_type, EventType.TRADE)
        self.assertEqual(envelope.source_sequence, None)
        self.assertEqual(envelope.instrument.canonical_instrument_id, "BTCUSDT")

    def test_stream_depth_maps_sequence(self):
        message = {
            "channel": "spot@public.increase.depth.v3.api@BTCUSDT",
            "symbol": "BTCUSDT",
            "data": {"bidsList": [], "asksList": [], "fromVersion": "10", "toVersion": "11"},
            "sendtime": 1757592000001,
        }
        envelope = adapter().parse_stream_message(json.dumps(message))
        self.assertEqual(envelope.event_type, EventType.ORDER_BOOK)
    ...
    def test_stream_invalid_json_maps_to_canonical_invalid_error(self):
        with self.assertRaises(Exception) as raised:
            adapter().parse_stream_message("{")
        self.assertEqual(type(raised.exception).__name__, "_MEXCProviderFailure")
    ...
    def test_stream_subscription_and_message_limit(self):
        message = json.dumps({"channel": "spot@public.deals.v3.api@BTCUSDT", "symbol": "BTCUSDT",
                              "data": {"time": 1757592000000}, "sendtime": 1757592000001})
        ...
        self.assertEqual(request["method"], "SUBSCRIPTION")
        self.assertEqual(request["params"], ["spot@public.deals.v3.api@BTCUSDT"])
