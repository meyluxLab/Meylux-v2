...
class QuantWorkerHandler:
    ...
    async def __call__(self,envelope:QueueEnvelope)->None:
        ...
        raw=envelope.payload; candles=tuple(_candle(x) for x in raw["candles"])
        if not candles or not candles[-1].is_closed: raise ValueError("worker requires closed candles")
        await self._persistence.persist_orchestration(self._orchestrator.process(candles,self._config,previous_regime=raw.get("previous_regime")))
