...
from contracts.canonical.foundation import ProvenanceRef, ValidationIssue, validation_outcome
    ...
    try:
        async with pool.acquire() as conn:
            ...
            row=await conn.fetchrow(
                "SELECT event_id,provider_id,adapter_id,adapter_version,canonical_instrument_id,provider_instrument_id,event_type,event_time,received_at,acquisition_state,source_sequence,provenance_id,acquisition_method,payload_json FROM meylux.raw_acquisition_events WHERE acquisition_state='AVAILABLE' AND ($1::text IS NULL OR event_id=$1) ORDER BY event_time,event_id LIMIT 1",
                event_id,
            )
            if row is None:
                print("vertical slice: no governed AVAILABLE raw record was found; no fallback data was fabricated",flush=True)
                return 2
            envelope=envelope_from_row(row)
            structural_ok=envelope is not None
            temporal=validate_temporal_evidence(envelope)
            normalized=normalize(envelope)
            issues=list(temporal.outcome.issues)
            if normalized.valid:
                issues.extend(validate_market_semantics(market_values(normalized.value)).issues)
            outcome=validation_outcome(issues) if issues else normalized.result if not normalized.valid else validation_outcome(())
            if not normalized.valid and not issues:
            ...
            print(f"vertical slice: raw_event={envelope.event_id} normalized={normalized.result.value} quality={assessment.quality.quality_state.value} canonical_eligible={assessment.canonical_eligible}",flush=True)
