...
class QuantitativePersistence:
    ...
    async def persist_orchestration(self,result:QuantOrchestrationResult)->int:
        ...
        return inserted
    async def fetch_family(self,family:str,symbol:str,timeframe:str,*,limit:int=100)->list[Any]:
        ...
        return list(await self._connection.fetch(f"SELECT * FROM {self.TABLES[family]} WHERE symbol=$1 AND timeframe=$2 ORDER BY event_time,record_id LIMIT $3",symbol,timeframe,limit))
