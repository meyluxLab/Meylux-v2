...
class QuantitativeAPI:
    def __init__(self,persistence:QuantitativePersistence):
        self._persistence=persistence

    async def handle(self,method:str,path:str,query:Mapping[str,str])->APIResponse:
        if method.upper()!="GET":
            return APIResponse(405,json.dumps({"error":"read_only"},separators=(",",":")))
        ...
        try:
            ...
            return APIResponse(400,json.dumps({"error":"invalid_limit"},separators=(",",":")))
        ...
        try:
            ...
            return APIResponse(400,json.dumps({"error":"invalid_request","detail":str(exc)},separators=(",",":")))
        except Exception:
            return APIResponse(503,json.dumps({"error":"quantitative_data_unavailable"},separators=(",",":")))
